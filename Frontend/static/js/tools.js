  ...
  if (val.length >= 8) {
    apiCall('/api/barcode/scan/' + encodeURIComponent(val.trim()))
      .then(function(prod) {
        ...
        var _isMr = typeof currentLang !== 'undefined' && currentLang === 'mr';
        var _dName = (_isMr && prod.name_mr) ? prod.name_mr : prod.name;
        if (el) el.innerHTML = '<div style="background:#f0fdf4;border-radius:8px;padding:12px;border:1.5px solid #bbf7d0;">'
          + '<div style="font-weight:700;font-size:14px;">' + escHtml(_dName) + '</div>'
